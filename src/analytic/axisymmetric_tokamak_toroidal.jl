...
# (r, θ, ϕ): both angles are periodic. The minor radius is not, and its range is left unbounded —
# `minx¹` and `maxx¹` are not overridden here.
function GeometricBase.periodic(::AxisymmetricTokamakToroidalEquilibrium)
